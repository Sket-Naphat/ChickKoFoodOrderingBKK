...
$(document).ready(function () {
  ...
  $("#currentOrderPanels").on(
    "click",
    ".btn-update-finishedOrder",
    async function () {
      ...
      try {
        ...
        await updateDoc(orderDocRef, {
          finishedOrder: true,
          finishedOrderTime: finishedOrderTime,
        });
  ...
  $("#currentOrderPanels").on(
    ...
    async function () {
      ...
      try {
        ...
        await updateDoc(orderDocRef, {
          discharge: true,
          dischargeTime: grabDischargeTime,
          dischargeType: "Grab",
        });
  ...
  $("#currentOrderPanels").on("click", ".btn-update-discharge", function () {
    ...
    if (total != nonDischargetotal) {
      $("#total").text(
        `ยอดรวม: ${nonDischargetotal.toFixed(2)} THB  (จาก ${total.toFixed(
          2
        )}) THB`
      );
    } else {
  ...
  $(
    "#menuList , #menufriedChickenList, #menuMealList , #menuNoodleList, #menuSnackList, #menuToppingList, #menuDessertList, #menuBeverageList"
    ...
    if (!addtopping) {
      ...
      // Show modal
      const modal = new bootstrap.Modal(
        document.getElementById("addToppingModal")
      );
      modal.show();
  ...
  $("#calculateChangeBtn").click(function () {
    ...
    let totalAmount = parseFloat(
      $("#total").text().replace("ยอดรวม: ", "").replace(" THB", "")
    );
  ...
  $("#calculateChange").on("click", function () {
    ...
    if (!isNaN(received) && received >= total) {
      ...
      $("#changeResult")
        .text(`เงินทอน: ${change.toFixed(2)} THB`)
  ...
  $("#applyDiscountBtn").on("click", function () {
    ...
    if (!isNaN(totalValue)) {
      ...
      $("#totalWithDiscount").text(
        "ยอดรวม (ส่วนลด 10%): " + discountedRounded + " THB"
      );
...
function getCurrentOrder() {
  ...
  onSnapshot(ordersQuery, (snapshot) => {
    ...
    const isChanged =
      JSON.stringify(currentOrderCache) !== JSON.stringify(newOrderList);
    ...
    if (isChanged) {
      ...
      const remarkInputs = {};
      document
        .querySelectorAll("textarea[id^='orderRemark-']")
        .forEach((item) => {
    ...
    if (item.discharge === false || item.finishedOrder === false) {
      ...
      if (item.locationOrder === "forHere") {
        ...
        switch (item.tableNumber) {
          case "t1":
            tableNumberText = "โต๊ะ 1";
            break;
          case "t2":
            tableNumberText = "โต๊ะ 2";
            break;
          case "t3":
            tableNumberText = "โต๊ะ 3";
            break;
          case "t4":
            tableNumberText = "โต๊ะ 4";
            break;
          case "t5":
            tableNumberText = "โต๊ะ 5";
            break;
          case "t6":
            tableNumberText = "โต๊ะ 6";
            break;
          case "tw":
            tableNumberText = "รอโต๊ะ";
            break;
          default:
            tableNumberText = "รอโต๊ะ";
            break;
      ...
      if (item.locationOrder === "Grab") {
        dischargeButton =
          '<button type="button" class="btn ' +
          bgDischargeColor +
          ' btn-update-grab-discharge" data-id="' +
          item.id +
          '">ส่งแกรป</button>';
      }
                ...
                <h2 class="accordion-header">
                  <button class="accordion-button collapsed ${bgColor}" type="button" data-bs-toggle="collapse"
                    data-bs-target="#panelsStayOpen-${
                      item.id
                    }" aria-expanded="false" aria-controls="panelsStayOpen-${
        item.id
      }">
                    ชื่อ : ${item.customerName} | เวลาสั่งซื้อ : ${
        item.orderTime
      } | ${locationOrderText}
                  </button>
                </h2>
                <div id="panelsStayOpen-${
                  ...
                  <div class="accordion-body">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <select class="form-select me-2 table-select" style="width: auto;" data-id="${
                          item.id
                        }">
                            <option value="t1" ${
                              item.tableNumber === "t1" ? "selected" : ""
                            }>โต๊ะ 1</option>
                            <option value="t2" ${
                              item.tableNumber === "t2" ? "selected" : ""
                            }>โต๊ะ 2</option>
                            <option value="t3" ${
                              item.tableNumber === "t3" ? "selected" : ""
                            }>โต๊ะ 3</option>
                            <option value="t4" ${
                              item.tableNumber === "t4" ? "selected" : ""
                            }>โต๊ะ 4</option>
                            <option value="t5" ${
                              item.tableNumber === "t5" ? "selected" : ""
                            }>โต๊ะ 5</option>
                            <option value="t6" ${
                              item.tableNumber === "t6" ? "selected" : ""
                            }>โต๊ะ 6</option>
                            <option value="tw" ${
                              item.tableNumber === "tw" ? "selected" : ""
                            }>รอโต๊ะ</option>
                            <option value="ta" ${
                              item.tableNumber === "ta" ? "selected" : ""
                            }>กลับบ้าน</option>
                            <option value="grab" ${
                              item.tableNumber === "grab" ? "selected" : ""
                            }>Grab</option>
                        </select>
                        <button type="button" class="btn btn-primary btn-add-more-order" data-id="${
                          item.id
                        }" data-customer="${
        item.customerName
      }">เพิ่มเมนู</button>
                        ...
                        ${item.items
                          ...
                          <tr>
                            ...
                            <td>
                             ...
                             <td>
                                <button type="button" class="btn btn-danger btn-delete-item" data-id="${
                                  item.id
                                }" data-index="${index}" data-name="${
                              i.name
                            }">ลบ</button>
                            </td>
                          </tr>
                    ...
                    <div class="form-floating position-relative">
                        <textarea class="form-control pe-5" placeholder="ใส่หมายเหตุ" id="orderRemark-${
                          item.id
                        }" style="height: 100px">${item.remark}</textarea>
                        <label for="orderRemark-${item.id}">หมายเหตุ</label>
                        
                        <!-- ปุ่มบันทึกอยู่ข้างใน textarea -->
                        <button type="button" class="btn btn-secondary btn-save-remark" data-id="${
                          item.id
                        }"
                    ...
                    <br>
                    <div class="d-flex justify-content-between align-items-center">
                        <button type="button" class="btn btn-danger btn-update-delete" data-id="${
                          item.id
                        }">ลบ order</button>
                        <div>
                            <button type="button" class="btn ${bgFinishedOrderColor} btn-update-finishedOrder" data-id="${
        item.id
      }">ทำครบแล้ว</button>
                            ${dischargeButton}
                        </div>
...
function displayMenu(menuData) {
  ...
  const menufriedChickenContainer = document.getElementById(
    "menufriedChickenList"
  );
  ...
  if (existingItem) {
    ...
    if (existingItem.remark != addMenuRemark) {
      cartItems.push({
        ...
        name,
        price,
        quantity: Number(addMenuQTY),
        is_done: false,
        remark: addMenuRemark,
        itemDischarge: false,
      });
    } else {
...
function updateCart() {
  ...
  cartItems.forEach((item) => {
    ...
    const countElements = document.getElementsByClassName(
      "menuCount_" + item.id
    );
    Array.from(countElements).forEach((element) => {
...
async function deleteItemFromOrder(orderId, itemIndex) {
  try {
    ...
    if (orderDoc.exists()) {
      ...
      // อัปเดต UI
      $(`.btn-delete-item[data-id="${orderId}"][data-index="${itemIndex}"]`)
        .closest("tr")
        .remove();
    } else {
      Swal.fire("เกิดข้อผิดพลาด", "ไม่พบข้อมูลรายการในฐานข้อมูล", "error");
    }
  } catch (error) {
